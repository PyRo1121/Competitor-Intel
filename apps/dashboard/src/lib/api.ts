...
export async function search(q: string, mode: SearchMode = "auto") {
...
export async function getCompanyProfileClaims(
  idOrSlug: string,
): Promise<CompanyProfileClaimsResponse> {
  return fetchAPI<CompanyProfileClaimsResponse>(
    `/api/companies/${encodeURIComponent(idOrSlug)}/profile-claims`,
  );
}
...
export async function getAlertRules(): Promise<AlertRulesResponse> {
  return fetchAPI<AlertRulesResponse>("/api/alerts");
}

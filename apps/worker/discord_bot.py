...
def get_db_stats() -> dict:
    ...
    stats["funding_events"] = cursor.fetchone()[0]
...
def generate_daily_brief_embed() -> dict:
...
class IntelBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        @self.intel_group.command(name="daily", description="Generate and post daily intelligence brief")
        ...
        async def intel_status(interaction: discord.Interaction):
            ...
            embed.add_field(
                name="Database",
                value=f"Companies: {stats['companies']}\nEvents: {stats['events']}\nFunding: {stats['funding_events']}",
                inline=True
            )
            embed.add_field(
                name="Last 24h",
        ...
        async def intel_companies(interaction: discord.Interaction):
            ...
            lines = []
            for name, industry, stars, score, status in companies:
        ...
        async def intel_signals(interaction: discord.Interaction, limit: int = 5):
            ...
            for company, event_type, amount, source, created in signals:
                company = company or "Unknown"
                amt_str = f"${amount / 1_000_000:.1f}M" if amount else "Undisclosed"
                date_str = created[:10] if created else "recent"
        ...
        async def intel_search(interaction: discord.Interaction, query: str):
            ...
            if events:
                event_lines = []
                for company, event_type, amount, source in events:
                    company = company or "Unknown"
                    amt_str = f"${amount / 1_000_000:.1f}M" if amount else "Undisclosed"
    ...
    async def on_ready(self):
        ...
        logger.info("Commands: /intel daily | /intel status | /intel collect | /intel companies | /intel signals | /intel search")

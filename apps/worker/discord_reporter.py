...
def generate_discord_embeds() -> List[Dict]:
    ...
    main_embed = {
        ...
        "description": f"Top signals from the last 24 hours across 16 AI companies",
        "color": 0x00ffaa,
...
def send_to_discord(webhook_url: str, content: Optional[str] = None) -> bool:
    """
    ...
    payload = {
        "content": content or "📡 **Competitor Intelligence Update**",
        "embeds": embeds
    }
...
if __name__ == "__main__":
    ...
    logger.info("  - Top companies: %s", [e['title'].split('—')[0].strip() for e in payload['embeds'][:3]])
...
def run_script(script_path: str, args: list[str] | None = None) -> tuple[bool, str]:
...
def cmd_daily(args):
    ...
    success, output = run_script("apps/worker/daily_brief.py", ["--export"] if args.export else [])
    if success:
...
def cmd_status(_args):
    ...
    cursor.execute(
        "SELECT COUNT(*) FROM raw_signals WHERE detected_at >= datetime('now', '-24hours')"
    )
    stats["signals_24h"] = cursor.fetchone()[0]

    cursor.execute(
        "SELECT COUNT(*) FROM intelligence_events WHERE created_at >=datetime('now', '-24 hours')"
    )
    stats["events_24h"] = cursor.fetchone()[0]

    cursor.execute("SELECT COUNT(*) FROM company_candidates WHERE status = 'pending'")
...
def cmd_companies(args):
    ...
    for name, industry, stars, score, _status in rows:
        score_str = f"{score:.1f}" if score else "N/A"
        stars_str = f"⭐{stars:,}" if stars else ""
        logger.info(
            "  %-20s | Score: %-6s | %s | %s", name, score_str, industry or "Unknown", stars_str
        )
    return 0
...
def cmd_signals(args):
    ...
    for company, event_type, amount, _source, created in rows:
        company = company or "Unknown"
        amt_str = f"${amount / 1_000_000:.1f}M" if amount else "Undisclosed"
...
def cmd_promote(_args):
    ...
    success, output = run_script(COLLECTORS["promote"])
    logger.info("Output:\n%s", output)
    return 0 if success else 1
...
def cmd_discover(_args):
    ...
    success, output = run_script(COLLECTORS["discover"])
    logger.info("Output:\n%s", output)
    return 0 if success else 1
...
def cmd_rank(_args):
    ...
    success, output = run_script(COLLECTORS["rank"])
    logger.info("Output:\n%s", output)
    return 0 if success else 1
...
Examples:
  ...
  python intel.py process                Process raw signals
    ...
    collect_parser.add_argument(
        "--collector", choices=list(COLLECTORS.keys()), help="Run specific collector"
    )

    # report
    report_parser = subparsers.add_parser("report", help="Generate reports")
    report_parser.add_argument(
        "--type", dest="report_type", choices=list(REPORTS.keys()), help="Generate specific report"
    )

...
app.get("/", zValidator("query", companyQuery), (c) => {
	...
	const orderBy =
		sort === "score" ? "score DESC NULLS LAST, name ASC" : "name ASC";
...
app.get("/:id", (c) => {
	...
	const details = db.prepare("SELECT * FROM company_details WHERE company_id = ?").get(id);
	const valuation = db
		.prepare(
	...
	const products = db
		.prepare(`SELECT * FROM products WHERE company_id = ? ORDER BY launch_date DESC`)
		.all(id);
	const team = db
		.prepare(`SELECT * FROM team_members WHERE company_id = ? ORDER BY joined_date DESC`)
		.all(id);
	const github = db
		.prepare(
  ...
  `,
		...
		.get(id, id, id, id, id, id, id, id, id, id) as Record<string, number>;

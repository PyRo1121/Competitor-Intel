...
def test_discovery_promote_rank_flow(operational_db, monkeypatch):
    ...
    from collectors.auto_promote import auto_promote_candidates, PROMOTION_THRESHOLD
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT score, status FROM company_candidates WHERE name LIKE '%Nebula Labs%'"
    )
    row = cur.fetchone()
...
def test_auto_promote_idempotent_when_company_exists(operational_db):
    ...
    cur.execute(
        ...
        VALUES ('Existing Co', 'test', 'unit', '{}', 0.9, '{}', 'pending', datetime('now'), datetime('now'))
        """
    )
    conn.commit()
    conn.close()

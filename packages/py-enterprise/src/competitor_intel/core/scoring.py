...
class ScoringEngine:
    ...
    def _score_engineering(self, session, company_id: int) -> float:
        ...
        metrics = session.execute(
            select(GitHubMetrics).where(
                GitHubMetrics.company_id == company_id
            ).order_by(GitHubMetrics.extracted_at.desc()).limit(1)
        ).scalar_one_or_none()
    ...
    def _score_social(self, session, company_id: int) -> float:
        ...
        activity = session.execute(
            select(XActivity).where(
                XActivity.company_id == company_id
            ).order_by(XActivity.extracted_at.desc()).limit(1)
        ).scalar_one_or_none()
    ...
    def _score_hiring(self, session, company_id: int) -> float:
        ...
        recent_hires = session.execute(
            select(func.count(TeamMember.id)).where(
                ...
                TeamMember.joined_date >= datetime.utcnow() - timedelta(days=90),

...
class EventExtractor:
    ...
    FUNDING_KEYWORDS = [
        ...
        "series seed", "round", "investment", "invested", "valuation",
        "led by", "participated", "investors", "funded",
    ]
    
    ACQUISITION_KEYWORDS = [
        "acquired", "acquisition", "buying", "bought", "merge", "merger",
        "acquires", "takeover",
    ]
    
    PRODUCT_KEYWORDS = [
        "launch", "released", "announced", "new feature", "new product",
        "introducing", "unveiled", "rolled out", "ga", "general availability",
    ]
    
    PRICING_KEYWORDS = [
        "pricing", "price increase", "price cut", "new pricing",
        "pricing update", "plan changes",
    ]
    ...
    def extract_from_signals(self, limit: int = 50) -> list[IntelligenceEvent]:
        ...
        logger.info("extraction_complete", signals_processed=len(signals), events_extracted=len(events))
        return events
    
    def _extract_event(self, signal: RawSignal) -> Optional[IntelligenceEvent]:
    ...
    def _classify_event(self, text: str) -> Optional[str]:
        ...
        if self._has_keywords(text_lower, rumor_indicators) and self._has_keywords(text_lower, self.FUNDING_KEYWORDS):
            return EventType.RUMORED_ROUND.value
        
        return None
    
    def _compute_confidence(self, text: str, event_type: str) -> float:
    ...
    def _extract_funding_fields(self, event: IntelligenceEvent, text: str, data: dict):
        ...
        # Extract round type
        round_types = ["seed", "series a", "series b", "series c", "series d", "series e", "pre-seed"]
    ...
    def _extract_date(self, data: dict) -> Optional[datetime]:

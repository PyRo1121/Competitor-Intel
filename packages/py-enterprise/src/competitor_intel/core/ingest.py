...
class IngestionService:
    ...
    def _is_duplicate(
        ...
        existing = session.execute(
            select(RawSignal).where(
                ...
                RawSignal.detected_at >= datetime.now(UTC) - timedelta(days=self.dedup_window_days),
            )
        ).scalar_one_or_none()
    ...
    def _create_raw_signal(
        ...
        return RawSignal(
            ...
            data_json=signal,
            detected_at=signal.get("detected_at", datetime.now(UTC)),
    ...
    def get_unprocessed_signals(
        ...
        with get_session() as session:
            query = select(RawSignal).where(RawSignal.processed.is_(False))

...
class KnowledgeGraph:
        ...
        with get_session() as session:
            ...
            rounds = session.execute(
                select(FundingRound).where(
                    FundingRound.lead_investor.contains(investor_name)
                )
            ).scalars().all()
    ...
    def get_tech_trends(self, category: Optional[str] = None) -> dict:
    ...
    def _get_team(self, session, company_id: int) -> dict:
        ...
        recent_hires = session.execute(
            select(TeamMember).where(
                TeamMember.company_id == company_id,
                TeamMember.joined_date >= datetime.utcnow().replace(month=1, day=1),
    ...
    def _get_tech_stack(self, session, company_id: int) -> list[dict]:
        ...
        stacks = session.execute(
            select(TechnologyStack).where(
                TechnologyStack.company_id == company_id

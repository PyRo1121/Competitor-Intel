...
class AlertEngine:
    ...
    def process_unalerted_events(self, limit: int = 100) -> list[dict]:
        ...
        with get_session() as session:
            alerted_ids = session.execute(
                select(AlertSent.event_id)
            ).scalars().all()
            
            query = select(IntelligenceEvent).where(
                IntelligenceEvent.id.notin_(alerted_ids) if alerted_ids else True
            ).order_by(
    ...
    def _format_alert_message(self, rule: AlertRule, event: IntelligenceEvent) -> str:

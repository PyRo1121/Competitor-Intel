...
class WebScraperCollector(BaseCollector):
    ...
    async def collect(self) -> list[dict[str, Any]]:
        ...
        companies = [
            ...
            {"name": "Anthropic", "url": "https://anthropic.com", "blog": "https://www.anthropic.com/news"},
            {"name": "Perplexity", "url": "https://perplexity.ai", "blog": "https://blog.perplexity.ai"},
            {"name": "Linear", "url": "https://linear.app", "blog": "https://linear.app/blog"},
            {"name": "Notion", "url": "https://notion.so", "blog": "https://www.notion.so/blog"},
            {"name": "ElevenLabs", "url": "https://elevenlabs.io", "blog": "https://elevenlabs.io/blog"},
    ...
    async def _scrape_blog(self, company: dict) -> list[dict[str, Any]]:
        ...
        for link in links:
            ...
            if any(p in href for p in ["/blog/", "/news/", "/post/", "/article/"]):
        ...
        try:
            ...
            if prices_found:
                signals.append({
                    ...
                    "summary": f"Price points found: {', '.join(f'${p}' for p in prices_found[:5])}",
                    "url": pricing_url,
                    "source": "web_scraper",
                    "signal_type": SignalType.PRODUCT_UPDATE.value,
                    "detected_at": datetime.utcnow().isoformat(),
        ...
        try:
            ...
            if team_size or location:
                signals.append({
                    ...
                    "summary": f"Team size: {team_size or 'N/A'}, Location: {location or 'N/A'}",
                    "url": about_url,
                    "source": "web_scraper",

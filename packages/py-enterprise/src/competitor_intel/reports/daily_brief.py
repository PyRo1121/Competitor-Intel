...
class DailyBriefReporter:
    ...
    def generate(self) -> dict[str, Any]:
        ...
        with get_session() as session:
            ...
            top_companies = session.query(Company).filter(
                Company.score.isnot(None)
            ).order_by(Company.score.desc()).limit(10).all()
            
            # Mega rounds in last 30 days
            mega_rounds = session.query(IntelligenceEvent).filter(
                IntelligenceEvent.event_type == "mega_round",
                IntelligenceEvent.created_at >= today - timedelta(days=30),
            ).order_by(IntelligenceEvent.amount_usd.desc().nulls_last()).all()
            
            # Rumored rounds
            rumors = session.query(IntelligenceEvent).filter(
                IntelligenceEvent.is_rumor == True,
    ...
    def _get_events(self, session, hours: int = 0, days: int = 0) -> list:
        ...
        return session.query(IntelligenceEvent).filter(
    ...
    def _get_trending_companies(self, session, days: int = 7) -> list[dict]:
        ...
        events = session.query(IntelligenceEvent).filter(
            IntelligenceEvent.created_at >= cutoff,
        ...
        return sorted(
            [{"name": d["name"], "event_count": d["count"], "event_types": list(d["types"])} for d in company_events.values()],
    ...
    def _get_funding_activity(self, session, days: int = 30) -> dict:
        ...
        rounds = session.query(FundingRound).filter(
    ...
    def _get_hiring_trends(self, session, days: int = 30) -> dict:
        ...
        active = session.query(JobPosting).filter(
            JobPosting.is_active == True
        ).count()
        
        new_postings = session.query(JobPosting).filter(
            JobPosting.posted_at >= cutoff
        ).count()
        
        # Top hiring companies
        top_hiring = session.query(
            Company.name,
            session.query(func.count(JobPosting.id))
            .filter(JobPosting.company_id == Company.id, JobPosting.is_active == True)
            .label("job_count"),
        ).filter(
            session.query(func.count(JobPosting.id))
            .filter(JobPosting.company_id == Company.id, JobPosting.is_active == True)
    ...
    def _get_executive_moves(self, session, days: int = 30) -> list[dict]:
        ...
        moves = session.query(TeamMember, Company).join(
            Company, TeamMember.company_id == Company.id
        ).filter(
            TeamMember.joined_date >= cutoff
        ).order_by(TeamMember.joined_date.desc()).limit(10).all()
    ...
    def _generate_analysis(self, events_7d, funding_activity, hiring_trends) -> dict:
        ...
        hiring_trend = "aggressive" if hiring_total > 50 else "moderate" if hiring_total > 20 else "slow"
    ...
    def export_csv(self, brief: dict[str, Any]) -> Path:
        ...
        with open(filepath, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                "company", "event_type", "amount_usd", "confidence",
                "source", "date", "is_rumor",
            ])
    ...
    def export_markdown(self, brief: dict[str, Any]) -> Path:
        ...
        lines.extend([
            f"",
            f"## Trending Companies",
            f"",
        ])
        
        for t in brief.get("trending", [])[:5]:
            lines.append(f"- **{t['name']}**: {t['event_count']} events ({', '.join(t['event_types'])})")
        ...
        for c in brief.get("top_companies", [])[:5]:
            lines.append(f"{c['rank']}. **{c['name']}** — Score: {c['score']:.1f} ({c['industry']})")
        ...
        for e in brief.get("recent_events", [])[:10]:
            ...
            lines.append(f"- **{e['company']}**: {e['type']}{amount} (confidence: {e['confidence']:.0%})")

...
class DiscordReporter:
    ...
    def generate_brief(self) -> dict[str, Any]:
        ...
        with get_session() as session:
            ...
            recent_funding = session.query(FundingEvent).order_by(
                FundingEvent.announced_date.desc()
            ).limit(5).all()
            
            # Recent intelligence events
            recent_events = session.query(IntelligenceEvent).order_by(
        ...
        if recent_events:
            events_text = "\n".join([
                f"• **{e.company.name if e.company else 'Unknown'}** — "
                f"{e.event_type}"

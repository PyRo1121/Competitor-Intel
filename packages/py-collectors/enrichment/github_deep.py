...
def analyze_company_github(company_id: int, github_org: str) -> bool:
    ...
    # Calculate metrics
    primary_lang = max(languages, key=languages.get) if languages else top_repo.get("language")
    ...
    try:
        ...
        conn.commit()
        logger.info("GitHub metrics stored for %s: %d commits/30d, %d contributors, %s",
                   github_org, commits_30d, contributors.get("total", 0), primary_lang)
        return True

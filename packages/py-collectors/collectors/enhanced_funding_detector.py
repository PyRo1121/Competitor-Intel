...
from collectors.enrichment.funding_aggregator import cluster_key
...
def extract_amount_enhanced(text: str) -> int | None:
    """Extract any dollar amount from text with comprehensive patterns."""
    if not text:
        return None

    shared = parse_amount_usd(text)
    ...
    m = re.search(
        r"([\d.]+)\s*(?:billion|b)\s+(?:dollar|deal|investment|round)", text, re.IGNORECASE
    )
    if m:
        try:
            return int(float(m.group(1)) * 1_000_000_000)
...
def extract_strategic_investors(text: str) -> list[str]:
    ...
    m = re.search(
        r"(?:led\s+by|backed\s+by|investment\s+from)\s+([A-Z][A-Za-z0-9&\s]{2,50}?)(?:,|;|\.|\s+and|\s+with|$)",
        text,
    )
    if m:
...
def process_signal_for_funding(
    signal_id: int, company_id: int | None, source: str, data_json: str
) -> dict | None:
    ...
    valuation = None
    m = re.search(
        r"(?:valuation|valued)\s+(?:at|of)\s+\$?([\d.]+)\s*(billion|million)", text, re.IGNORECASE
    )
    if m:
        try:
            num = float(m.group(1))
...
def store_enhanced_funding_event(data: dict) -> bool:
    ...
    # Deduplication key
    dedup = (
        f"{data['company_id']}:{data['event_type']}:"
        f"{data.get('amount_usd', 'unknown')}:{hash(data['text_preview']) % 10000}"
    )
    ...
    try:
        ...
        # Also store in funding_rounds (cluster_key so rollup prune_legacy_rounds keeps rows)
        if data.get("company_id") and (data.get("amount_usd") or data.get("valuation_usd")):
            ...
            cursor.execute(
                ...
                (company_id, round_type, amount_usd, valuation_usd, lead_investor,
                 ...
                 extracted_at, cluster_key, report_count)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
                """,
                (
                    data["company_id"],
        ...
        conn.commit()
        logger.info(
            "Stored %s event: %s | %s | Amount: %s",
            data["event_type"],
            data.get("lead_investor") or "Unknown",
...
def run_enhanced_funding_detection(batch_size: int = 200) -> dict:
    ...
    if strict_pipeline_blocks_legacy_events("enhanced_funding_detector"):
    ...
    for row in signals:
        ...
        if result:
            if store_enhanced_funding_event(result):
                ...
                stats["by_type"][result["event_type"]] = (
                    stats["by_type"].get(result["event_type"], 0) + 1
                )
            else:
    ...
    logger.info(
        "Funding scan complete: %d signals, %d funding events found",
        stats["processed"],
        stats["funding_found"],
    )

...
from collectors.enrichment.embedding_generator import cosine_similarity, get_embedding
...
def rerank_companies(query: str, top_k: int = 10) -> list[dict]:
    ...
    cursor.execute("""
        SELECT c.id, c.name, c.industry, c.github_stars, c.score,
               cd.description_long, cd.business_model, cd.tech_stack,
               COALESCE(cd.embedding, c.embedding) AS embedding
        FROM companies c
...
def rerank_events(query: str, top_k: int = 10) -> list[dict]:
    ...
    cursor.execute("""
        SELECT ie.id, ie.company_id, c.name, ie.event_type, ie.amount_usd,
               ie.confidence, ie.source, ie.source_url, ie.created_at, ie.embedding
